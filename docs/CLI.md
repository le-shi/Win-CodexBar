...
| `codexbar_reset_credits_next_expiry_timestamp_seconds{provider="codex"}` | Unix timestamp of the earliest expiry among currently available reset credits. Omitted when no credit is available, the API does not provide an expiry, or reset-credit data is unavailable. |
